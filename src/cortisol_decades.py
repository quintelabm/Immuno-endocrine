...
def cortisolDecadesOneDay(simulation, cortisol_exp):
    ...
    for i in range(
        ...
        cortisol_wcsa = pd.DataFrame(outputs_wcsa[7], columns=pd.Series(["values"]))
        ...
        # 1440000 data points
        gluc_intake_gi = pd.DataFrame(outputs_gi[11], columns=pd.Series(["values"]))  # Glucose itake
        # print("Tamanho glicose GI: ", np.size(gluc_intake_gi))#720000
        last_value = gluc_intake_gi["values"].iat[-1]
        temp_gluc_gi = pd.DataFrame(np.repeat(last_value, 720000, axis=0), columns=pd.Series(["values"]))
        ...
        if i == (days - 1):
            ...
            data = [
                i + 1,
                avg_cor,
                cortisol_gi["values"].max(),
                cortisol_gi["values"].min(),
                cortisol_gi["values"].std(),
            ]
            with open(folder + out_filename, "a+") as f:
                writer = csv.writer(f)
                writer.writerow(data)
...
def cortisolDecadesOneWeek(
    simulation: Literal["F", "M"], cortisol_exp: float, gluc_intake_decade_multiplier: float, simulation_days: int
):
    print(f"Simulation started! ({simulation_days} days)")
    print("Loading files...")
    # create new file
    ...
    # sim_time_wcsa = 1         # day        # Total time of simulation in min
    deltaT_wcsa = pow(10, -3)  # Step size
    t_wcsa = np.arange(0, simulation_days, deltaT_wcsa)
    ...
    gluc_intake_wcsa = pd.concat([gluc_intake_wcsa] * simulation_days, axis=0)  # 7 days = 70000
    ...
    gluc_intake_wcsa["values"] = gluc_intake_wcsa["values"] * gluc_intake_decade_multiplier

    [t_wcsa, outputs_wcsa] = week_csa.Week_Cortisol_Cytokines_SAureus(simulation_days, gluc_intake_wcsa, ic)

    # write on file
    week_csa.save_output(folder, "bacteria.csv", outputs_wcsa[0], simulation_days)
    week_csa.save_output(folder, "ma.csv", outputs_wcsa[1], simulation_days)
    week_csa.save_output(folder, "mr.csv", outputs_wcsa[2], simulation_days)
    week_csa.save_output(folder, "il10.csv", outputs_wcsa[3], simulation_days)
    wcsa.save_output(folder, "il6.csv", outputs_wcsa[4], simulation_days)
    wcsa.save_output(folder, "il8.csv", outputs_wcsa[5], simulation_days)
    week_csa.save_output(folder, "TNF.csv", outputs_wcsa[6], simulation_days)
    week_csa.save_output(folder, "cortisol.csv", outputs_wcsa[7], simulation_days)
    week_csa.plots_w_c_sa(t_wcsa, folder, outputs_wcsa, simulation_days)
